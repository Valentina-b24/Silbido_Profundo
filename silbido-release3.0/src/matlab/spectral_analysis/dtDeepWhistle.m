...
if ~isscalar(blksize)
    ...
    % Controllo di sicurezza: se è vuoto o invalido, mettiamo un valore standard
    if isempty(blksize) || isnan(blksize) || blksize <= 0
        blksize = 1024; 
    end
    blksize = round(blksize);

    % CREIAMO IL LAYER (Questa riga è vitale!)
    layer = imageInputLayer([blksize, 1], 'Name', 'Input_input.1', 'Normalization', 'none');
