...
if ~all(blksize == inputsize(1:2))
            ...
            % --- CORREZIONE VALENTINA ---
            dim_sicura = double(blksize(1));
            if isempty(dim_sicura) || isnan(dim_sicura) || dim_sicura <= 0
                dim_sicura = 1024; 
            end
            layer = imageInputLayer([round(dim_sicura), 1], 'Name', 'Input_input.1', 'Normalization', 'none', 'NormalizationDimension', 'auto');
            % --- FINE CORREZIONE ---
